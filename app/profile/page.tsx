...
import { useRestaurantsContext } from "@/components/RestaurantsProvider";
...
export default function ProfilePage() {
  ...
  const { reloadWithNewPrefs } = useRestaurantsContext();

  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  // États locaux des prefs — null = pas encore modifié (on lit depuis profile)
  ...
  function toggle<T>(list: T[], setList: (v: T[]) => void, id: T) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Relance le top 5 avec les nouvelles prefs, puis redirige
      await reloadWithNewPrefs(updated);
      router.push("/");
    } catch (err) {
      ...
      <Section title="Mes cuisines préférées">
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
          {CUISINES.map((c) => (
            <TagChip
              key={c.id}
              active={currentCuisines.includes(c.id)}
              onClick={() =>
      ...
      <Section title="Types de lieu">
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
          {VENUE_TYPES.map((v) => (
            <TagChip
              key={v.id}
              active={currentVenues.includes(v.id)}
              onClick={() =>
      ...
      <Section title="Mon budget">
        <div style={{ display: "flex", gap: "8px" }}>
          {PRICE_LEVELS.map((p) => (
            <PillButton
              key={p.id}
              active={currentPrices.includes(p.id)}
              onClick={() =>
